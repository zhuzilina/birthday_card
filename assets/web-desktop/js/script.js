...
document.addEventListener("DOMContentLoaded", function () {
	...
	function initFullscreenFab() {
		...
		fscreen.addEventListener("fullscreenchange", () => {
			if (isFullscreen()) {
				fab.classList.add('in-fullscreen');
				fab.title = '点击退出全屏模式';
				// 更新图标为退出全屏
				fab.querySelector('svg path').setAttribute('d', 'M5 16h3v3h2v-5H5v2zm3-8H5v2h5V5H8v3zm6 11h2v-3h3v-2h-5v5zm2-11V5h-2v5h5V8h-3z');
			} else {
				fab.classList.remove('in-fullscreen');
				fab.title = '点击进入全屏模式';
				// 恢复为进入全屏图标
				fab.querySelector('svg path').setAttribute('d', 'M7 14H5v5h5v-2H7v-3zm-2-4h2V7h3V5H5v5zm12 7h-3v2h5v-5h-2v3zM14 5v2h3v3h2V5h-5z');
			}
		});

		// 如果URL参数包含fullscreen=true，3秒后自动尝试进入全屏
		if (urlParams.get('fullscreen') === 'true') {
			setTimeout(() => {
				if (fullscreenEnabled() && !isFullscreen()) {
					...
					setTimeout(() => {
						fab.style.animation = '';
						fab.style.animation = 'breathe 1s ease-in-out 3';
					}, 10);
				}
			}, 3000);
		}
...
Object.keys(appNodes).forEach((key) => {
	const element = document.querySelector(appNodes[key]);
	if (element) {
		appNodes[key] = element;
	} else {
		console.warn(`Element not found: ${appNodes[key]} for key: ${key}`);
	}
});
...
function renderApp(state) {
	if (appNodes.controls && appNodes.controls.classList) {
		appNodes.controls.classList.toggle("hide", state.menuOpen || state.config.hideControls);
	}
	if (appNodes.canvasContainer && appNodes.canvasContainer.classList) {
		appNodes.canvasContainer.classList.toggle("blur", state.menuOpen);
	}
	if (appNodes.menu && appNodes.menu.classList) {
	...
	if (appNodes.wordShell) appNodes.wordShell.checked = state.config.wordShell;
	...
	if (appNodes.helpModal && appNodes.helpModal.classList) appNodes.helpModal.classList.toggle("active", !!state.openHelpTopic);
	if (state.openHelpTopic) {
		const { header, body } = helpContent[state.openHelpTopic];
		if (appNodes.helpModalHeader) appNodes.helpModalHeader.textContent = header;
		if (appNodes.helpModalBody) appNodes.helpModalBody.textContent = body;
	}
}
...
//音效管理器
const soundManager = {
	baseURL: typeof getApiUrl === 'function' ? getApiUrl('audio/') : "./audio/",
	...
	preload() {
		...
		if (window.location.protocol === 'http:' || window.location.protocol === 'https:') {
			console.log('HTTP(S) environment detected, loading audio via fetch');
		...
		return this._waitForBase64Audio()
			.then(() => {
				console.log('Base64 audio data available, attempting to load');
				return this._loadBase64Audio();
			})
			.catch(error => {
				console.warn('Failed to load Base64 audio, falling back to fetch:', error);
				return this._loadFetchAudio();
			});
	},
	...
	_loadBase64Audio() {
		...
		types.forEach((type) => {
			...
			fileNames.forEach((fileName) => {
				const base64Data = window.audioData[fileName];

				if (base64Data) {
					console.log('Loading Base64 audio:', fileName);
					const promise = fetch(base64Data)
						.then(response => response.arrayBuffer())
						.then(data => {
							return new Promise((resolve, reject) => {
								this.ctx.decodeAudioData(data, resolve, (error) => {
									console.error('Failed to decode Base64 audio:', fileName, error);
									resolve(null);
								});
							});
						})
						.catch(error => {
							console.error('Failed to load Base64 audio:', fileName, error);
							return null;
						});
					...
					console.warn('Base64 audio data not found for:', fileName);
				}
			});
			...
			Promise.all(filePromises).then((buffers) => {
				...
				if (validBuffers.length > 0) {
					source.buffers = validBuffers;
					console.log('✓ Base64 audio buffers loaded for type:', type, validBuffers.length + '/' + fileNames.length);
				} else {
					console.warn('✗ No valid Base64 audio buffers loaded for type:', type);
				}
			});
		});
	...
	_loadFetchAudio() {
		console.log('Loading audio via fetch...');
		const allFilePromises = [];
	...
	playSound(type, scale = 1) {
		// 在HTTP环境下允许播放声音
		if (this._browserMode && (window.location.protocol === 'http:' || window.location.protocol === 'https:')) {
			// 继续执行播放逻辑
		} else if (this._browserMode) {
			return;
		}
...
if (IS_HEADER) {
	...
	setTimeout(() => {
		...
		Promise.all(promises).then(() => {
			...
			console.log("音频资源加载失败，继续初始化:", reason);
			init();
